...
async function downloadFile(url) {
    try {
        ...
        consola.log(`File ${url} downloaded successfully.`)
...
function getRequired(required = [], propertyName) {
    return Array.isArray(required) && required.includes(propertyName) ? '<span class="required-tag"></span>' : ''
}
...
function generateMarkdownDoc(doc, helmMode) {
...
# ${escapeHtml(kind)}\n\n`
  ...
  <div class="crd-actions">
    ...
    // Spec section
    if (!helmMode && schemaObj.properties && schemaObj.properties.spec) {
    ...
    // Status section
    if (!helmMode && schemaObj.properties && schemaObj.properties.status) {
    ...
    if (helmMode) {
        markdown += generatePropertyTable(schemaObj.properties, false, 0, '', schemaPrefix)
    }
...
async function main() {
    const helmValuesPath = `${PREFIX}/charts/tensor-fusion/values.schema.json`
    const helmValues = await downloadFile(helmValuesPath)
    const helmDoc = JSON.parse(helmValues)
    const helmDocMarkdown = generateMarkdownDoc({
        kind: "Helm Values",
        spec: {
            names: {
                kind: "Helm Values",
    ...
    fs.writeFileSync(
        path.join(DOCS_DIR, "../helm-install-values.md"),
        helmDocMarkdown
    )

    consola.success(`Generated documentation for helm values completed`)

    for (const schema of schemaList) {
        // Download and parse YAML
        const yamlContent = await downloadFile(schema.url)
        const doc = yaml.load(yamlContent)


        // Generate markdown documentation
        const markdown = generateMarkdownDoc(doc)
